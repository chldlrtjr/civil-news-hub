...
### ① 공모전 카드 상단 뱃지 배치
- **1행 (상단 헤더)**: 좌측에 `[카테고리 뱃지]`만 단독 배치하고, **우측 최상단에는 상태 뱃지(`[접수중]`)를 배제하고 직관적인 D-Day 뱃지(`[D-Day]`, `[접수예정]`, `[상시접수]`) 단독 배치(`flex justify-between`)**하여 시각적 군더더기를 없앤다.
- **2행 (접수 기간)**: `[📅 접수기간: YYYY.MM.DD ~ MM.DD]` 뱃지는 1행 아래에 독립적인 행으로 배치하여, 긴 기간 텍스트로 인해 뱃지가 줄바꿈되거나 밀리지 않도록 한다.
...
- **현재 기준 버전**: **`v1.0.44`** (2026.09.12 기준 / 사용자 누적 수정 44회 반영)
- **패치 버전 (+0.0.1) 자동 증가**:
