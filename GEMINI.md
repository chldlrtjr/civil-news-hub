...
### ① 공모전 카드 상단 뱃지 배치
- **1행 (상단 헤더)**: 좌측에 `[카테고리 뱃지]`만 단독 배치하고, **우측 최상단에 상태 뱃지(`[접수중]`, `[접수예정]`, `[상시접수]`, `[접수마감]`) 및 `[D-Day]` 뱃지를 함께 배치(`flex justify-between`)**한다. (좌측 카테고리 영역에 D-Day를 두지 않고 우측 상태 영역으로 통일 배치하여 시각적 위계 확립)
...
- **현재 기준 버전**: **`v1.0.43`** (2026.09.12 기준 / 사용자 누적 수정 43회 반영)
- **패치 버전 (+0.0.1) 자동 증가**:
