...
// PWA Service Worker 등록 (업데이트 감지 시 자동 캐시 삭제 + 리프레시)
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js').then((reg) => {
      // SW 업데이트 감지 → 즉시 활성화 후 리로드
...
function showNewsLoading(show) {
  ...
  if (loading) {
    if (show) {
      ...
      // 15초 안전 타임아웃: 무한 로딩 원천 방지
      if (window._newsLoadingSafetyTimer) clearTimeout(window._newsLoadingSafetyTimer);
      window._newsLoadingSafetyTimer = setTimeout(() => {
        showNewsLoading(false);
        console.warn('[Safety] 뉴스 로딩 15초 초과 — 스피너 강제 해제');
      }, 15000);
    } else {
      loading.style.display = 'none';
      loading.classList.add('hidden');
      loading.classList.remove('flex');
      if (window._newsLoadingSafetyTimer) {
        clearTimeout(window._newsLoadingSafetyTimer);
        window._newsLoadingSafetyTimer = null;
      }
    }
  }
  if (container) {
